...
import {
  Brain, AlertTriangle, CheckCircle2, Compass, Sparkles, Zap, BadgeCheck,
} from "lucide-react";

/**
 * The "shows its mind" panel (Chat Page Spec §6) — the trust differentiator.
 * Live multi-dimensional mastery, misconceptions caught as they happen
 * (open -> fixed in-session), what the tutor remembers, and the next step.
 */
...
const STATE_LABEL = {
  ...
  assessing: "Assessing",
  relearning: "Re-teaching the gap",
  mastered: "Mastered 🎉",
  needs_help: "Let's simplify",
};
...
export default function TutorMind({ mind, usage }) {
  if (!mind) {
    return (
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs font-extrabold text-slate-700">
              {STATE_LABEL[mind.state] || "Learning"}
              {mind.attempt_no > 1 && <span className="ml-1 text-slate-400">· attempt {mind.attempt_no}</span>}
            </p>
            <p className="mt-0.5 text-[11px] font-bold text-slate-400">
              {mind.topic_mastered
                ? "Every concept passed — topic mastered!"
                : "Mastered when every concept passes 80%."}
            </p>
          </div>
        </div>
      ...
      {usage && (
        <Section icon={<Zap className="h-3.5 w-3.5 text-amber-500" />} title="AI credits">
          <div className="flex items-center justify-between">
            <p className="text-[11px] font-extrabold text-slate-600">
              {usage.daily.used} of {usage.daily.limit} today
            </p>
            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-extrabold text-slate-500">
              <BadgeCheck className="h-3 w-3" /> {usage.plan}
            </span>
          </div>
          <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-slate-100">
            <div
              className={`h-full rounded-full transition-all ${
                usage.daily.used / Math.max(1, usage.daily.limit) > 0.85
                  ? "bg-gradient-to-r from-rose-400 to-rose-500"
                  : "bg-gradient-to-r from-amber-400 to-amber-500"
              }`}
              style={{ width: `${Math.min(100, (usage.daily.used / Math.max(1, usage.daily.limit)) * 100)}%` }}
            />
          </div>
          <p className="mt-1 text-[10px] font-bold text-slate-400">Resets at midnight · {usage.monthly.used}/{usage.monthly.limit} this month</p>
        </Section>
      )}
    </div>
  );
