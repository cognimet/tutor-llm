    ...
    <>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500">Where students are struggling — and whether the loop is closing those gaps.</p>
        </div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">
        <Card className="p-5">
          <p className="text-sm font-extrabold uppercase tracking-wide text-slate-500">Open gaps by severity</p>
              ...
              return (
                <div key={s}>
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-extrabold capitalize text-slate-600">{s}</span>
        ...
        <Card className="p-5 lg:col-span-2">
          <p className="text-sm font-extrabold uppercase tracking-wide text-slate-500">Created vs resolved</p>
      ...
      <Card className="mt-6 p-5">
        <p className="text-sm font-extrabold uppercase tracking-wide text-slate-500">Most-failed concepts (content weak spots)</p>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        {/* Students at risk */}
        <Card className="p-5">
          <p className="text-sm font-extrabold uppercase tracking-wide text-slate-500">Students needing attention</p>
          <div className="mt-3 space-y-2">
            ...
            {data.students_at_risk.map((s, i) => (
              <div key={s.user?.id ?? i} className="flex items-center justify-between rounded-2xl border border-slate-100 p-3">
                <div className="min-w-0">
                  <p className="truncate text-sm font-extrabold text-slate-800">{s.user?.name || "Unknown"}</p>
        ...
        <Card className="p-5">
          <p className="text-sm font-extrabold uppercase tracking-wide text-slate-500">Cohort mastery by topic (weakest first)</p>
          <div className="mt-3 space-y-3">
            ...
            {data.mastery_by_topic.map((m) => (
              <div key={m.topic_name}>
                <div className="flex items-center justify-between text-sm">
                  <span className="min-w-0 flex-1 truncate font-extrabold text-slate-700">{m.topic_name}</span>
      ...
      <Card className="mt-6 p-5">
        <p className="text-sm font-extrabold uppercase tracking-wide text-slate-500">Misconceptions the tutor keeps catching</p>
        <div className="mt-3 space-y-2">
          ...
          {data.misconceptions.map((m, i) => (
            <div key={i} className="flex items-start justify-between gap-3 rounded-2xl border border-slate-100 p-3">
              <div className="min-w-0">
                <p className="text-sm font-bold leading-snug text-slate-700">{m.description}</p>
                <p className="text-xs text-slate-400">{m.topic_name}</p>
              </div>
              <div className="flex shrink-0 gap-1.5">
                <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-extrabold text-slate-500">{m.count}×</span>
