        ...
        <div className="auth-rise mx-auto w-full max-w-[400px]">
          ...
          <div className="mb-8 flex items-center gap-2.5 lg:hidden">
            ...
            <div className="leading-tight">
              <p className="font-display text-lg font-extrabold tracking-tight text-slate-900">Tuto<span className="text-indigo-500">.ai</span></p>
          ...
          <div className="relative flex rounded-2xl bg-slate-100 p-1">
            <span
              className="absolute inset-y-1 left-1 w-[calc(50%-0.25rem)] rounded-xl bg-white shadow-sm ring-1 ring-slate-200/70 transition-transform duration-300 ease-out"
              style={{ transform: isRegister ? "translateX(100%)" : "translateX(0)" }}
            />
            {["login", "register"].map((m) => (
              <button key={m} type="button" onClick={() => { setMode(m); setError(""); }}
                className={`relative z-10 flex-1 rounded-xl py-2.5 text-sm font-bold transition-colors ${mode === m ? "text-indigo-600" : "text-slate-500 hover:text-slate-700"}`}>
          ...
          <div className="mt-7">
            <h1 className="font-display text-[1.7rem] font-extrabold leading-tight tracking-tight text-slate-900">
                  ...
                  {ROLES.map((r) => {
                    ...
                    return (
                      <button type="button" key={r.id} onClick={() => setRole(r.id)}
                        className={`group relative flex items-center gap-3 rounded-xl border p-3 text-left transition-all ${active ? "border-indigo-500 bg-indigo-50/70 ring-1 ring-indigo-500" : "border-slate-200 hover:border-slate-300 hover:bg-slate-50"}`}>
                        <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg transition-colors ${active ? "bg-indigo-500 text-white" : "bg-slate-100 text-slate-500 group-hover:bg-slate-200"}`}>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2.5">
              {DEMOS.map((d) => (
                <button key={d.email} onClick={() => demoLogin(d.email)} disabled={busy}
                  className="group flex flex-col items-center gap-1.5 rounded-xl border border-slate-200 py-3 text-xs font-bold text-slate-600 transition-all hover:-translate-y-0.5 hover:border-indigo-300 hover:text-indigo-600 hover:shadow-md disabled:opacity-50">
                  <span className="grid h-8 w-8 place-items-center rounded-lg bg-slate-100 text-slate-500 transition-colors group-hover:bg-indigo-100 group-hover:text-indigo-600">
...
function FieldLabel({ children }) {
  return (
    <span className="mb-1.5 flex items-center gap-1.5 text-[13px] font-semibold text-slate-700">{children}</span>
  );
}
  ...
  return (
    <label className="block">
      ...
      <div className="relative">
        ...
        <input
          {...props}
          className={`h-12 w-full rounded-xl border border-slate-200 bg-white text-[15px] font-medium text-slate-800 outline-none transition-all placeholder:font-normal placeholder:text-slate-400 hover:border-slate-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 ${Icon ? "pl-10" : "pl-3.5"} ${trailing ? "pr-12" : "pr-3.5"}`}
        />
