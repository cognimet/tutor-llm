          ...
          {(report.usage.last_7_days || []).length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {report.usage.last_7_days.map((a) => (
                <span key={a.action_type} className="rounded-full bg-slate-50 px-3 py-1.5 text-xs font-extrabold capitalize text-slate-600 ring-1 ring-slate-200">
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="font-extrabold text-slate-800">{Number(used).toLocaleString()} / {Number(limit).toLocaleString()}</span>
      </div>
  ...
  return (
    <Card className="mt-6 p-5">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-5">
        <MiniStat label="Open gaps" value={s.open ?? 0} tone="text-slate-900" />
        ...
        <div>
          ...
          <div className="mt-2 space-y-3">
            ...
            {(dash.mastery_by_topic || []).map((m) => (
              <div key={m.topic_name} className="rounded-2xl border border-slate-100 p-3">
                <div className="flex items-center justify-between text-sm">
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {(m.concepts || []).map((c) => (
                    <span key={c.concept} className={`rounded-full px-2 py-0.5 text-[10px] font-extrabold ${c.passes ? "bg-emerald-50 text-emerald-600" : "bg-slate-50 text-slate-500"}`}>
                      {c.concept} · {c.score}%
                    </span>
                  ))}
                </div>
